...
async function addTextBlock(
    ...
    const description = intl.formatMessage({
        id: 'CardDetail.addCardText',
        defaultMessage: 'add card text',
    })
...
function moveBlock(
    card: Card,
    srcBlock: IContentBlockWithCords,
    ...
    const srcBlockY =
        (srcBlock.cords.y || srcBlock.cords.y === 0) && srcBlock.cords.y > -1
            ? srcBlock.cords.y
            : -1
    const dstBlockY =
        (dstBlock.cords.y || dstBlock.cords.y === 0) && dstBlock.cords.y > -1
            ? dstBlock.cords.y
            : -1
    ...
    const newContentOrder = dragAndDropRearrange({
        contentOrder,
        srcBlockId,
        srcBlockX,
        srcBlockY,
    ...
    mutator.performAsUndoGroup(async () => {
        const description = intl.formatMessage({
            id: 'CardDetail.moveContent',
            defaultMessage: 'Move card content',
        })
...
const ContentBlockWithDragAndDrop = (
    props: ContentBlockWithDragAndDropProps
) => {
    const [, isOver, , itemRef] = useSortableWithGrip(
        'content',
        { block: props.block, cords: { x: props.x } },
        true,
        (src, dst) => moveBlock(props.card, src, dst, props.intl, 'aboveRow')
    )
    const [, isOver2, , itemRef2] = useSortableWithGrip(
        'content',
        { block: props.block, cords: { x: props.x } },
        true,
        (src, dst) => moveBlock(props.card, src, dst, props.intl, 'belowRow')
    )
                ...
                <div style={{ display: 'flex' }}>
                    {props.block.map((b, y) => (
                        <ContentBlock
                            ...
                            width={
                                (1 /
                                    (props.block as ContentBlockType[])
                                        .length) *
                                100
                            }
